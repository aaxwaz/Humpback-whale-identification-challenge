...
def get_branch_model(inp_shape):
    from classification_models.senet import SEResNeXt50, preprocess_input
    from keras.models import Model
    from keras.layers import GlobalMaxPooling2D
    model = SEResNeXt50(input_shape=inp_shape, include_top=False, weights='imagenet')
...
def strong_aug(p=.5):
    return Compose([
        HorizontalFlip(p=0.002),
        VerticalFlip(p=0.002),
        RandomRotate90(p=0.002),
        OneOf([
        ...
        ShiftScaleRotate(shift_limit=0.02, scale_limit=0.05, rotate_limit=30, p=0.9),
        OneOf([
            CLAHE(clip_limit=2),
            IAASharpen(),
            IAAEmboss(),
        ...
        JpegCompression(p=0.2, quality_lower=45, quality_upper=99),
...
def preprocess_image(img):
    from classification_models.senet import preprocess_input
